...
// 아래 주석의 자바 코드를 안드로이드 스튜디오에 복붙하면, 자동으로 코틀린으로 변환해준다..ㄷㄷ
 ...
 * @author scottm
 */
class IntListVer3 @JvmOverloads constructor(initialCap: Int = DEFAULT_CAP) {
    //instance variables
    // iValues store the elements of the list and 
    // may have extra capacity
    private var iValues: IntArray
     ...
     * @param x
     */
    fun insert(pos: Int, x: Int) {
        assert(0 <= pos && pos <= size()) {
            "Failed precondition insert. " +
                    "pos is invalid. Value of pos: " + pos
        }
        ensureCapcity()
        for (i in iSize downTo pos + 1) {
            iValues[i] = iValues[i - 1]
        }
        iValues[pos] = x
        iSize++
    ...
    fun remove(pos: Int): Int {
        assert(0 <= pos && pos < size()) {
            "Failed precondition remove. " +
                    "pos it out of bounds. Value of pos: " + pos
        }
        val removedValue = iValues[pos]
        for (i in pos until iSize - 1) iValues[i] = iValues[i + 1]
        iSize--
        return removedValue
    }

    private fun ensureCapcity() {
        // is there extra capacity available?
        // if not, resize
        if (iSize == iValues.size) resize()
    }
    ...
    // resize internal storage container by a factor of 2
    private fun resize() {
        val temp = IntArray(iValues.size * 2)
        System.arraycopy(iValues, 0, temp, 0, iValues.size)
        iValues = temp
    }
    ...
    override fun toString(): String {
        // we could make this more effecient by using a StringBuffer.
        // See alternative version
        var result = "size: $iSize, elements: ["
        for (i in 0 until iSize - 1) result += iValues[i].toString() + ", "
        if (iSize > 0) result += iValues[iSize - 1]
        result += "]"
        return result
    }
    ...
    // Would not really have this and toString available
    // both included just for testing
    fun toStringUsingStringBuffer(): String {
        ...
        if (iSize > 0) result.append(iValues[iSize - 1])
        result.append("]")
        return result.toString()
    }

    /**
     * Return true if this IntList is equal to other.<br></br>
     * pre: none
     * @param other The object to comapre to this
     * @return true if other is a non null, IntList object
     * that is the same size as this IntList and has the
    ...
    override fun equals(other: Any?): Boolean {
        var result: Boolean
        if (other == null) // we know this is not null so can't be equal
            result = false else if (this === other) // quick check if this and other refer to same IntList object
            result = true else if (this.javaClass != other.javaClass) // other is not an IntList they can't be equal
            result = false else {
            // other is not null and refers to an IntList
            val otherIntList = other as IntListVer3
            result = size() == otherIntList.size()
            var i = 0
            while (i < iSize && result) {
                result = iValues[i] == otherIntList.iValues[i]
                i++
            }
        }
        return result
    ...
    companion object {
        // class constant for default size
        private const val DEFAULT_CAP = 10
    }
    /**
     * Constructor to allow user of class to specify
     * initial capacity in case they intend to add a lot
    ...
    init {
        assert(initialCap > 0) {
            ("Violation of precondition. IntListVer1(int initialCap):"
                    + "initialCap must be greater than 0. Value of initialCap: " + initialCap)
        }
        iValues = IntArray(initialCap)
        iSize = 0
    }
}
